/* Prebuilt cockpit demo — a faithful vanilla port of the design handoff's own
...
function vBars(groups,unit){ // groups: [{label, bars:[{label?,value,color,dim?}]}]
  ...
  groups.forEach((g,gi)=>{ const n=g.bars.length, bw=Math.min(64,(gw-24)/n);
    g.bars.forEach((b,bi)=>{ const x=pad+16+gi*gw+bi*(bw+6), h=(b.value/maxV)*(H-84), y=H-44-h;
      out+=`<rect x="${x}" y="${y}" width="${bw}" height="${h}" rx="1" fill="${b.dim?"var(--series-dim)":b.color||"var(--series-1)"}"><title>${esc(g.label)}${b.label?" · "+esc(b.label):""}: ${fmt(b.value,unit)}</title></rect>`;
...
function heat(cols,rowsY,val,unit,inv){
  ...
  rowsY.forEach((yv,j)=>{ out+=`<text x="232" y="${26+j*ch+ch/2+4}" text-anchor="end" class="val">${esc(String(yv).slice(0,30))}</text>`;
    cols.forEach((x,i)=>{ const v=val(x,yv); let t=(v-mn)/((mx-mn)||1); if(inv)t=1-t;
      ...
      out+=`<rect x="${240+i*cw}" y="${26+j*ch}" width="${cw-4}" height="${ch-4}" rx="1" fill="${HEAT[step]}"/><text pointer-events="none" x="${240+i*cw+cw/2-2}" y="${26+j*ch+ch/2+2}" text-anchor="middle" fill="${step>=3?"var(--surface-card)":"var(--ink-900)"}">${fmt(v,unit)}</text>`;});});
